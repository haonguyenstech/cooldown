#!/bin/bash
# Cooldown — public installer. Always installs the latest release.
#
...
# Cooldown drives ~/cooldown.sh for the calm/restore actions.
if [ ! -f "$HOME/cooldown.sh" ]; then
  echo ""
  echo "ℹ️  Note: ~/cooldown.sh was not found — the Calm Down / Undo Calm buttons need it."
  echo "    (The live CPU gauge and Start-at-login still work without it.)"
fi

open "$DEST/$APP"
echo ""
echo "✅ Done! Look for the thermometer icon in your menu bar."
echo "   Tip: toggle 'Start at login' in the popover so it launches automatically."
